...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    switch (event) {
    //スキャン応答データセットが完了すると、イベントが発生します
    case ESP_GAP_BLE_SCAN_RSP_DATA_SET_COMPLETE_EVT:
        adv_config_done &= (~SCAN_RSP_CONFIG_FLAG);
        if (adv_config_done == 0){
            //アドバタイス開始  引数adv_paramsデータ構造体
            esp_ble_gap_start_advertising(&heart_rate_adv_params);
        }
        break;
    //アドバタイズデータのセットが完了すると、イベントが発生します
    ...
    case ESP_GAP_BLE_SEC_REQ_EVT:
        ...
        break;
    //IOに出力機能があり、ピアデバイスIOに入力機能がある場合、アプリはこのevtを受信します。    
                                        ...
                                        esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    switch (event) {
        case ESP_GATTS_REG_EVT: //アプリケーションID登録
            ...
            break;
        case ESP_GATTS_READ_EVT:    //GAT読み取り要求
            printf("GAT読み取り要求\n");
            break;
        case ESP_GATTS_WRITE_EVT:   //GAT書き込み要求
            ESP_LOGI(GATTS_TABLE_TAG, "ESP_GATTS_WRITE_EVT, write value:");
            esp_log_buffer_hex(GATTS_TABLE_TAG, param->write.value, param->write.len);
            break;
        case ESP_GATTS_EXEC_WRITE_EVT:  //gattクライアントリクエストが書き込みを実行
            printf("GATクライアントリクエスト書き込み要求\n");
            break;
        case ESP_GATTS_MTU_EVT:     //mtuが完了すると、イベントが発生します パケットの上限サイズがMTU   BLE 4.0においてMTUは20バイトです。そして、BLE 4.2においてMTUは257バイトになりました。　実際は150バイトくらい
            printf("MTU完了\n");
            break;
        case ESP_GATTS_CONF_EVT:    //確認を受け取ると、イベントが発生します
            printf("確認\n");
            break;
        case ESP_GATTS_UNREG_EVT:   //アプリケーションIDの登録を解除すると、イベントが発生します
            printf("ID登録解除\n");
            break;
        case ESP_GATTS_DELETE_EVT:  //サービスの削除が完了すると、イベントが発生します
            printf("サービス削除\n");
            break;
        case ESP_GATTS_START_EVT:   //サービス開始が完了すると、イベントが発生します
            printf("サービス開始\n");
            break;
        case ESP_GATTS_STOP_EVT:    //サービス停止が完了すると、イベントが発生します
            printf("サービス停止\n");
            break;
        case ESP_GATTS_CONNECT_EVT: //gattクライアントが接続すると、イベントが発生します
            ESP_LOGI(GATTS_TABLE_TAG, "ESP_GATTS_CONNECT_EVT");
            /* start security connect with peer device when receive the connect event sent by the master */
            esp_ble_set_encryption(param->connect.remote_bda, ESP_BLE_SEC_ENCRYPT_MITM);
            break;
        case ESP_GATTS_DISCONNECT_EVT://gattクライアントが切断されると、イベントが発生します
            ...
            esp_ble_gap_start_advertising(&heart_rate_adv_params);
            break;
        case ESP_GATTS_OPEN_EVT:    //ピアに接続すると、イベントが発生します
            /*再接続*/
            printf("再接続\n");
            esp_ble_set_encryption(param->connect.remote_bda, ESP_BLE_SEC_ENCRYPT_MITM);
            break;
        case ESP_GATTS_CANCEL_OPEN_EVT: //ピアから切断すると、イベントが発生します
            printf("ピア切断\n");
            break;
        case ESP_GATTS_CLOSE_EVT:   //gattサーバーが閉じると、イベントが発生します
            printf("GAT閉じる\n");
            break;
        case ESP_GATTS_LISTEN_EVT:  //ガットが接続を聞くと、イベントが発生します
            printf("GAT接続開く\n");
            break;
        case ESP_GATTS_CONGEST_EVT: //混雑が発生すると、イベントが発生します
            printf("混雑\n");
            break;
        case ESP_GATTS_CREAT_ATTR_TAB_EVT: {    //gatt create tableが完了すると、イベントが発生します
...
void ble_init(void)
{
    ...
    // Initialize NVS.　このAPIは、デフォルトのNVSパーティションを初期化します。デフォルトのNVSパーティションは、パーティションテーブルで「nvs」とラベル付けされているパーティションです。
    ret = nvs_flash_init();
    ...
    ESP_ERROR_CHECK( ret );
    //esp_bt_controller_mem_release モードに従ってコントローラーメモリを解放します
    ESP_ERROR_CHECK(esp_bt_controller_mem_release(ESP_BT_MODE_CLASSIC_BT));

    esp_bt_controller_config_t bt_cfg = BT_CONTROLLER_INIT_CONFIG_DEFAULT();
    //BTコントローラーを初期化して、タスクやその他のリソースを割り当てます。この関数は、他のBT関数が呼び出される前に、一度だけ呼び出す必要があります。
    ...
    //ブルートゥースのリソースを初期化して割り当てます。すべてのブルートゥースのものの前にある必要があります。
    ret = esp_bluedroid_init();
    if (ret) {
        ESP_LOGE(GATTS_TABLE_TAG, "%s init bluetooth failed: %s", __func__, esp_err_to_name(ret));
        return;
    }
    //Bluetoothを有効にします。esp_bluedroid_init（）の後にする必要があります。
    ...
    //この関数は、アプリケーションのコールバックをBTAGATTSモジュールに登録するために呼び出されます。
    ret = esp_ble_gatts_register_callback(gatts_event_handler);
    if (ret){
        ESP_LOGE(GATTS_TABLE_TAG, "gatts register error, error code = %x", ret);
        return;
    }
    //この関数は、スキャン結果などのギャップイベントを発生させるために呼び出されます。
    ret = esp_ble_gap_register_callback(gap_event_handler);
    if (ret){
        ESP_LOGE(GATTS_TABLE_TAG, "gap register error, error code = %x", ret);
        return;
    }
    //この関数は、アプリケーション識別子を登録するために呼び出されます。
    ...
    /*GAPセキュリティパラメータ値を設定します。デフォルト値を上書きします。
    「ピンペアリングプロトコルの偽装」（CVE-2020-26555）や「LEレガシーペアリングプロトコルの認証」などの主要な脆弱性を回避するために、安全な接続を強くお勧めします。
    パラメーター
    [in] param_type：：設定するパラメータの種類
    [in] value：：パラメータ値
    [in] len：：パラメータ値の長さ*/
